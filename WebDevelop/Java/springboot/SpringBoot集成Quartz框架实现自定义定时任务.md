...
在Spring中实现定时任务大致有三种方式:

1. Java自带的Timer类,优点主要是简单易用吧,缺点应该就是不易集中管理.
2. SpringBoot自带的TimeTask,也就是@Schedule注解实现的方式,优点是同样的简单易用,好配置,但是缺点是不能实现我的自定义定时任务的需求,需要提前配置好.
3. 第三种就是Quartz了.优点是专注于任务调度,方便新增修改删除,可以统一管理,缺点当然是配置繁琐,集成过程操作较多.
...
Quartz是一个任务调度框架,可以有效的实现任务调度的功能
...
2. 编写`web-quartz.properties`配置文件,放在`classpath`目录下
...
其中使用@Autowired注入前面配置类创建的Scheduler,使用该调度器来执行操作.
...
完成以上内容之后,就能在业务中使用QuartzJobManager进行任务的创建,修改,删除等操作了,
看下面这个例子,是我业务中,在新增系统消息时,根据参数判断是否是定时发送的,是的话,就新建一个MessageQuartz的任务,将发送时间进行格式化为cron表达式,创建一次性的定时发送任务
...
https://blog.csdn.net/u010827544/article/details/106524607

https://michael728.github.io/2020/09/06/java-middleware-quartz-basic/

[在线Cron表达式](https://cron.qqe2.com/)

https://github.com/CodingDocs/springboot-guide/blob/master/docs/advanced/SpringBoot-ScheduleTasks.md